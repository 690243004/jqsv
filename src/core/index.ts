...
class Jqsv {
  ...
  async processData(): Promise<any> {
    ...
    try {
      this.sTime = Date.now()
      const response = await onLoad(this.sTime)
  ...
  processEvent(): void {
    ...
    const dragStart = <T = TouchEvent | MouseEvent>(e: T) => {
    ...
    const dragMove = <T = TouchEvent | MouseEvent>(e: T) => {
      if (!isTouch) return
    ...
    const dragEnd = <T = TouchEvent | MouseEvent>(e: T) => {
      if (!isTouch) return
      isTouch = false
      let eventX = 0
      if (e instanceof TouchEvent) {
      ...
      onSubmit(
        ...
        .catch(() => {
          ...
          block.css({ transform: `translateX(${toPx(globalConfig.initialX)})` }) // 重置滑块
        })
    }
    ...
    return new Promise((resolve) => {
      ...
      componentWraper
        ...
        .animate(
          ...
          500,
          crossText.fadeIn.bind(null, 500, resolve)
        )
    })
  }
}
